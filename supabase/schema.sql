...
--   * uptime_verdicts rows are publicly readable, because that is the entire integrity claim:
...
-- ---------------------------------------------------------------------------
create table if not exists uptime_documents (
    id            uuid primary key default gen_random_uuid(),
    doc_hash      text not null,
    user_wallet   text,
    created_at    timestamptz not null default now(),

    constraint uptime_documents_doc_hash_unique unique (doc_hash)
);

alter table uptime_documents enable row level security;
...
-- ---------------------------------------------------------------------------
create table if not exists uptime_extractions (
    id                uuid primary key default gen_random_uuid(),
    document_id       uuid references uptime_documents(id) on delete cascade,
    payload           jsonb not null,
    document_quality  integer,
    created_at        timestamptz not null default now()
);

alter table uptime_extractions enable row level security;
-- No public read.
...
-- ---------------------------------------------------------------------------
create table if not exists uptime_verdicts (
    id             uuid primary key default gen_random_uuid(),
    document_id    uuid references uptime_documents(id) on delete set null,
    ...
    constraint uptime_verdicts_verdict_hash_unique unique (verdict_hash)
);

alter table uptime_verdicts enable row level security;
